...
- **Fallback chains (owner-approved):** open-weight trio is primary; `google/gemini-3-flash-preview` sits in the Tier-B fallback chain for structured-JSON reliability, and `anthropic/claude-haiku-4.5` is the break-glass Tier-C fallback for provider outages/refusal spikes on villain personas. All model IDs live in one config file — swappable without code changes.
